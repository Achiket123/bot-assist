...
func handleCommand(chatId int64, command string) error {
	...
	case "/auth":
		url, err := auth.Auth(fmt.Sprintf("%v", chatId), "telegram")
		if err != nil {
			log.Printf("An error occured: %s", err.Error())
		}
		if url != "" {
			msg := tgbotapi.NewMessage(chatId, "Please click the link below to authenticate:")
			msg.ParseMode = tgbotapi.ModeMarkdown
			msg.Text = fmt.Sprintf("[%s](%s)", "Authenticate", url)
			_, err = bot.Send(msg)
			if err != nil {
				log.Printf("An error occured: %s", err.Error())
			}
